...
void bst_insert(bst& b, bnode* n){
	...
	if(b==NULL) b = n;
	    ...
	    while (x != NULL) {
			y = x;
			if (compare_key(get_key(n), get_key(x)) < 0) x = get_left(x);
			else x = get_right(x);
	    }
	    n->parent = y;
	    if (compare_key(get_key(n), get_key(y))<0) y->left = n;
        else y->right = n;
	}
}
...
void bst_insertShort(bst& b, bnode* n){
	bnode* x = b; // x is a copy of b
	bnode* y = NULL;
	if(b==NULL) b = n;
	else {
	    while (x != NULL) {
...
void bst_delete(bst& b, bnode* n){
	bnode* new_child; // variabile d'appoggio che individua il nodo da sostuire a n come figlio
	   if (get_left(n) == NULL) {
	    ...
	    if(new_child != NULL) new_child->parent = get_parent(n);

	    if(n==b) b=new_child; // n è la radice
	    else update_father(n, new_child);
